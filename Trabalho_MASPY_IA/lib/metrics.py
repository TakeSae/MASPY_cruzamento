...
class MetricsCollector:
    ...
    def gerar_relatorio(self):
        ...
        for nome_agente, metricas in self.metricas_por_agente.items():
            ...
            # Pular agentes sem dados de treinamento (veiculos que so participam da coordenacao)
            if num_episodios == 0 and total_acoes == 0:
                agentes_sem_treino.append(nome_agente)
                continue
        ...
        # Listar agentes sem treinamento (participaram apenas da coordenacao)
        if agentes_sem_treino:
            relatorio.append(f"  {aviso(f'Agentes sem metricas de treinamento ({len(agentes_sem_treino)}):')}")
            relatorio.append(f"  (Participaram apenas da fase de coordenacao multi-agente)")
            for nome in agentes_sem_treino:
                relatorio.append(f"    - {nome}")
            relatorio.append("")

        # Resumo comparativo apenas com agentes que treinaram
        agentes_treinados = {
            nome: m for nome, m in self.metricas_por_agente.items()
            if nome not in agentes_sem_treino
        }
        if len(agentes_treinados) > 1:
            relatorio.append("\n" + "="*70)
            relatorio.append(titulo("ANÁLISE COMPARATIVA"))
            relatorio.append("="*70 + "\n")

            ranking = sorted(
                agentes_treinados.items(),
